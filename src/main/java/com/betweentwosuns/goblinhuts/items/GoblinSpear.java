...
public class GoblinSpear extends Item implements Vanishable {
	private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;

	public static final GoblinSpear GOBLIN_SPEAR = new GoblinSpear();
	...
	public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
		GoblinHuts.LOGGER.info("Released goblin spear?");
		user.playSound(SoundEvents.ENTITY_ZOMBIE_HURT, 1.0f, 1.0f);
		...
		if (j > 0) {
			...
			if (playerEntity.isOnGround()) {
				playerEntity.move(MovementType.SELF, new Vec3d(0.0, 2f, 0.0));
			}
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (false) {
			GoblinHuts.LOGGER.info("FIRST USE CHECK");
